...
# ---------------- CONFIGURATION ------------------------------------------------------------
NUM_PROC = 16                          # adjust based on available cpu cores
WORKER_CACHE_SIZE = 8192 * 1024 * 1024  # keep a small cache per worker to prevent I/O thrashing
WRITER_BATCH_SIZE = 1                 # number of rows per write op for the .map() cache file writer
# -------------------------------------------------------------------------------------------

# Global cache dictionary specifically for the isolated worker processes
WORKER_CACHE = {}
# This counter lives independently inside each spawned worker process
...
def process_slice_worker(example):
    ...
    # Print a status update every 1000 slices processed by THIS specific core
    ...
    # # Check memory every 500 slices
    # if WORKER_LOCAL_COUNTER % 500 == 0:
    #     snapshot = tracemalloc.take_snapshot()
    #     top_stats = snapshot.statistics('lineno')
    #     print(f"[Worker] Top 9 memory blocks:")
    #     for stat in top_stats[:9]:
    #         print(stat)
