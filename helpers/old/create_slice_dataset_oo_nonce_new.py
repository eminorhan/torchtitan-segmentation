...
import numpy as np
import io
import numcodecs # Keep this to kill the thread explosion
...
# ---------------- CONFIGURATION ------------------------------------------------------------
NUM_PROC = 80                          
# WORKER_CACHE_SIZE is GONE. We are reading bare-metal.
# -------------------------------------------------------------------------------------------
...
def slice_generator(zarr_paths, _process_index=0, _num_processes=1):
    # Keep the thread killer: stops CPUs from fighting each other
    ...
    for zarr_path in my_paths:
        try:
            # THE FIX: Absolute bare-metal read. 
            # No LRU cache. No store. Just open it directly.
        ...
        for axis in [0, 1, 2]:
            for slice_idx in range(shape[axis]):
                ...
                # Notice what is missing here? No gc.collect(). No cache.clear().
                # We let Python naturally destroy `slice_2d` and `buffer` on the next loop iteration.
